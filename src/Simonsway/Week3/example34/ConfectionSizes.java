package solutions.Simon.example34;
...
public class ConfectionSizes {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            sex = inputKonsole.next().charAt (0); // hier konvertieren wir den String nach Char
            if (sex == 'h' || sex == 'd' ){       // Hier prüfen wir ob die Eingabe "h" oder "d" ist wenn du eingabe h ist dann startet das erst if und hier wird nochmal überprüft ob es h ist, wenn nicht geht es zu else

                if (sex == 'h') {
                    System.out.print ( "Geben sie die Größe an (44-70):" );
                    size = inputKonsole.nextInt ( );

                    if(size >=44 && size <=70 && size%2 == 0 ){ // mit Modulo können wir schauen ob es eine gerade Zahl ist
                        switch (size) {
                            case 44: // hier muss wirklich die Zahl eingegeben werden die Überprüft werden muss
                            case 46:
                    ...
                    System.out.print ( "Geben sie die Größe an (38-50):" ); // wenn h nicht eingeben wird dann fragen wir d ab
                    size = inputKonsole.nextInt ( );
